...
CREATE TABLE IF NOT EXISTS zone_deplacement (
    libelle_deplacement TEXT PRIMERY KEY
);
...
CREATE TABLE IF NOT EXISTS dureee_temps_partiel (
...
CREATE TABLE IF NOT EXISTS contact (
      ...
      fax TEXT,
      contact_mail TEXT,
      id_employeur TEXT NOT NULL REFERENCES employeur(id_employeur)
);
...
CREATE TABLE IF NOT EXISTS permis_requis (
      ...
      id_permis TEXT REFERENCES permis(id_permis),
      numero_offre TEXT REFERENCES offre(numero_offre),
...
CREATE TABLE IF NOT EXISTS langage_requis (
      ...
      libelle_langue REFERENCES langue(libelle_langue),
      niveau REFERENCES niveau_langue(libelle_niveau),
      numero_offre  REFERENCES offre(numero_offre),
      requis TEXT NOT NULL CHECK(requis in ('true', 'false'))
);
...
CREATE TABLE IF NOT EXISTS offre (
  ...
  id_commune INTEGERNOT NULL REFERENCES commune(id_commune),
  id_employeur INTEGER NOT NULL REFERENCES employeur(id_employeur),
  niveau_Formation REFERENCES niveau_formation(niveau_formation),
  diplome TEXT,
