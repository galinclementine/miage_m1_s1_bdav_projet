...
SELECT
    ...
    dtp.id_duree_temps_partiel
FROM source_csv t
    LEFT JOIN offre.contrat ct ON t.typeContrat = ct.libelle_contrat
    LEFT JOIN offre.employeur e ON t.employeur_type = e.type_employeur
        AND t.employeur_nomentreprise = e.nom_entreprise
        AND t.employeur_nompersphysique = e.nom_pers_physique
        AND t.employeur_prenompersphysique = e.prenom_pers_physique
        AND t.employeur_email = e.employeur_mail
    LEFT JOIN offre.commune c ON t.communeEmploi = c.libelle_commune
    LEFT JOIN offre.qualification q ON t.qualifications = q.libelle_qualification
    LEFT JOIN offre.duree_temps_partiel dtp ON substring(t.dureetempspartiel,'([0-9]{1,4})')::INTEGER = dtp.nombre_heure
        AND CASE
            WHEN t.dureetempspartiel like 'Au moins %' THEN 'min'
            WHEN t.dureetempspartiel like 'Moins de %' THEN 'max'
            WHEN t.dureetempspartiel = 'NaN' THEN 'NaN'
        END = dtp.seuil
    LEFT JOIN offre.duree_contrat dc ON t.duree::INTEGER = dc.nombre
        AND t.uniteduree = dc.type_duree
    LEFT JOIN offre.niveau_formation nf ON t.niveauFormation = nf.libelle_niveau_formation
    LEFT JOIN offre.experience ex ON t.experience = ex.libelle_experience;
